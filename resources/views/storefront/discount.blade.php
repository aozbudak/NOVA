...
@section('content')
    <div class="mx-auto max-w-[1600px] px-4 pt-10 md:px-8 md:pt-14">
        <p class="text-[11px] tracking-nav uppercase text-muted-foreground">{{ __('storefront.nav.discounts') }} / {{ $discount->name }}</p>
        <div class="mt-4 flex flex-wrap items-end justify-between gap-4">
            <div>
                <h1 class="font-serif text-4xl md:text-5xl">{{ $discount->name }}</h1>
                <p class="mt-2 text-xs tracking-wide text-muted-foreground">
                    {{ $discount->type->value === 'percent' ? '%'.(int) $discount->value : \Illuminate\Support\Number::currency((float) $discount->value, in: 'EUR') }}
                    · {{ __('storefront.listing.products_count', ['count' => $products->total()]) }}
                </p>
            </div>
            <div class="flex items-center gap-4">
                <form method="get" action="{{ route('discounts.show', $discount) }}" class="flex items-center gap-2">
                    @foreach (['size', 'color', 'price', 'collection', 'availability'] as $key)
                        @if (filled($filters[$key] ?? null))
                            <input type="hidden" name="{{ $key }}" value="{{ $filters[$key] }}">
                        @endif
                    @endforeach
                    <label class="sr-only" for="discount-sort">{{ __('storefront.listing.sort') }}</label>
                    <select id="discount-sort" name="sort" onchange="this.form.submit()" class="bg-transparent py-1 text-[11px] tracking-label uppercase outline-none">
        ...
        <div class="mt-10 grid gap-12 lg:grid-cols-[16rem_minmax(0,1fr)]">
            <aside class="hidden lg:block">
                <form method="get" action="{{ route('discounts.show', $discount) }}" class="flex flex-col gap-8">

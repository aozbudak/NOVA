...
<div data-admin-search hidden class="fixed inset-0 z-50" data-search-url="{{ route('api.admin.search') }}" data-search-groups='@json(__("admin.search.groups"))'>
    ...
    <div class="relative mx-auto mt-[12vh] w-full max-w-lg px-4">
        <div data-search-panel class="admin-popover overflow-hidden rounded-2xl border">
            <div class="flex items-center gap-2 border-b border-border px-3.5">
                ...
                <input
                    ...
                    autocomplete="off"
                >
                <button type="button" data-close-search class="rounded-xl p-1 text-muted-foreground hover:bg-muted hover:text-foreground">
                ...
                @foreach ($sections as $section)
                    @foreach ($section['items'] as $item)
                        <a
                            ...
                            class="flex items-center gap-2.5 rounded-xl px-2.5 py-2 text-[13px] text-foreground hover:bg-muted"
                        >
